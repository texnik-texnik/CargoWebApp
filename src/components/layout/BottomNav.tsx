  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-50 border-t bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      <div className={`grid h-16 ${isAdmin ? 'grid-cols-4' : 'grid-cols-3'}`}>
        <Link to="/home" className={`flex flex-col items-center justify-center gap-1 transition-colors ${pathname === '/home' ? 'text-primary' : 'text-muted-foreground hover:text-primary'}`}>
        ...
        <Link to="/" className={`flex flex-col items-center justify-center gap-1 transition-colors ${(pathname === '/' || pathname === '/profile') ? 'text-primary' : 'text-muted-foreground hover:text-primary'}`}>
