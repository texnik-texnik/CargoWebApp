...
export default function TracksPage() {
  ...
  useEffect(() => {
    ...
    if (tg?.initDataUnsafe?.user) {
      ...
      loadSearchHistory(String(tg.initDataUnsafe.user.id));
    } else {
  ...
  async function loadSearchHistory(telegramId: string) {
    try {
      ...
      if (data?.history) {
        setSearchHistory(data.history.split(',').filter(Boolean));
      }
    } catch (err) {
  ...
  async function handleSearch(code: string) {
    ...
    try {
      ...
      // Сохраняем в историю
      if (user) {
        await saveToHistory(user.id, code);
      } else {
  ...
  async function saveToHistory(telegramId: string, code: string) {
    try {
  ...
  // Сохраняем в localStorage для не-авторизованных пользователей
  ...
  return (
    <div className="container mx-auto px-4 py-6 max-w-2xl">
      ...
      <Card className="mb-6">
        <CardHeader>
          <CardTitle>Поиск трека</CardTitle>
          <CardDescription>
