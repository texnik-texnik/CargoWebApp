    ...
    return (
      <div className="container mx-auto px-4 py-6 max-w-2xl space-y-6">
        ...
        <div className="grid grid-cols-2 gap-3">
          {[1,2,3,4].map(i => <Card key={i}><CardContent className="pt-6"><Skeleton className="h-12 w-12 rounded-full mx-auto mb-2" /><Skeleton className="h-4 w-20 mx-auto" /></CardContent></Card>)}
        </div>
      </div>
    );
  }
                  ...
                  <CardContent className="p-4">
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">{track.updated_at ? new Date(track.updated_at).toLocaleDateString('ru-RU') : ''}</p>
                      </div>
